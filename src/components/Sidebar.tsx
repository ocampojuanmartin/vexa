    ...
    <>
          ...
          return (
            <button key={item.href} onClick={() => navigate(item.href)}
              className={`flex items-center gap-3 w-full px-3 py-2.5 rounded-lg text-sm transition-colors ${
                isActive ? 'sidebar-active text-vexa-700 font-medium' : 'text-gray-500 hover:bg-gray-50 hover:text-gray-900'
              }`}
      ...
      <div className="border-t border-gray-200 p-2 space-y-0.5">
        ...
        <div className="flex items-center gap-3 px-3 py-2">
          ...
          <button onClick={handleLogout} className="p-1.5 rounded-md hover:bg-gray-50 text-gray-400" title={t('auth.logout')}>
            <LogOut size={15} />
          </button>
        </div>
      </div>
