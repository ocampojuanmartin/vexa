...
type Matter = { id:string; title:string; status:string; is_restricted:boolean; is_billable:boolean; loading_language:string; requires_authorization:boolean; rate_currency:string; custom_rate:number|null; client_id:string }
...
const emptyMatterForm=():MatterForm=>({title:'',status:'active',is_billable:true,loading_language:'es',requires_authorization:false,rate_currency:'USD',is_restricted:false,restricted_associates:[],originators:[],category_rates:[]})
...
export default function ClientsPage(){
  ...
  async function openEditMatter(m:Matter,e:React.MouseEvent){
    ...
    const sb=createClient()
    const[orig,lawyers,cr]=await Promise.all([
    ...
    setMatterForm({
      ...
      rate_currency:m.rate_currency||'USD',
  ...
  async function saveMatter(){
    ...
    const payload={
      ...
      rate_currency:matterForm.rate_currency,custom_rate:null,firm_id:firmId,lead_lawyer_id:null,matter_type:'general',
    }
    let matterId:string
    if(editingMatter){
  ...
  const L={
    ...
    reqAuth:es?'Requiere autorización del cliente':'Requires client authorization',
    reqAuthDesc:es?'El abogado deberá indicar quién autorizó al cargar horas':'Lawyer must indicate who authorized when logging hours',
    rateCurrency:es?'Moneda de tarifa':'Rate currency',
    rateByCategory:es?'Tarifa por categoría':'Rate per category',
    rateByDesc:es?'Tarifa hora para cada categoría de abogado en este asunto':'Hourly rate for each lawyer category on this matter',
  ...
  return(
    <div>
      ...
      {clients.length>0&&(
        <div className="mt-4 relative"><Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400"/>
          <input type="text" value={search} onChange={e=>setSearch(e.target.value)} placeholder={L.search} className="w-full pl-9 pr-4 py-2.5 border border-gray-300 rounded-lg text-sm"/></div>
      )}
              ...
              <div key={client.id} className="bg-white rounded-xl border border-gray-200 overflow-hidden">
                ...
                {isOpen&&(
                  <div className="border-t border-gray-100 bg-gray-50/30 px-4 py-3">
                    {cm.length===0?<p className="text-sm text-gray-400 py-2">{L.noMatters}</p>:(
                      <div className="space-y-1.5">{cm.map(m=>(
                        <div key={m.id} className="flex items-center gap-3 px-3 py-2 bg-white rounded-lg border border-gray-100">
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            <span className="text-xs text-gray-400 ml-2">{m.rate_currency||'USD'}</span>
      ...
      {/* MATTER MODAL */}
      {showMatterModal&&(
        <div className="fixed inset-0 bg-black/40 flex items-start justify-center z-50 px-4 pt-8 overflow-y-auto">
          <div className="bg-white rounded-xl w-full max-w-lg p-6 shadow-xl mb-12">
            ...
            <div className="space-y-4">
              ...
              {/* Status + Currency + Language */}
              <div className="grid grid-cols-3 gap-3">
                <div><label className="block text-sm font-medium text-gray-700 mb-1">{L.status}</label>
                  <select value={matterForm.status} onChange={e=>setMatterForm({...matterForm,status:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">{STATUSES.map(s=><option key={s} value={s}>{statusLabel(s)}</option>)}</select></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">{L.rateCurrency}</label>
                  <select value={matterForm.rate_currency} onChange={e=>setMatterForm({...matterForm,rate_currency:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white">{CURRENCIES.map(c=><option key={c} value={c}>{c}</option>)}</select></div>
                <div><label className="block text-sm font-medium text-gray-700 mb-1">{L.loadLang}</label>
                  <select value={matterForm.loading_language} onChange={e=>setMatterForm({...matterForm,loading_language:e.target.value})} className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white"><option value="es">Español</option><option value="en">English</option></select></div>
              </div>
              ...
              {matterForm.category_rates.length>0&&(
                <div className="border-t pt-4 border-gray-100">
                  <label className="block text-sm font-medium text-gray-700 mb-1">{L.rateByCategory} ({matterForm.rate_currency})</label>
                  <p className="text-xs text-gray-400 mb-3">{L.rateByDesc}</p>
                  <div className="space-y-2">
                    {matterForm.category_rates.map((cr,i)=>{
                      const cat=categories.find(c=>c.id===cr.category_id)
                      return(
                        <div key={cr.category_id} className="flex items-center gap-3">
                          <span className="text-sm text-gray-700 flex-1">{cat?.name||'?'}</span>
                          <input type="number" step="1" value={cr.rate} onChange={e=>{const a=[...matterForm.category_rates];a[i].rate=e.target.value;setMatterForm({...matterForm,category_rates:a})}}
                            className="w-28 px-3 py-1.5 border border-gray-300 rounded-lg text-sm text-right" placeholder="0"/>
              ...
              <div className="border-t pt-4 border-gray-100">
                ...
                {matterForm.originators.map((o,i)=>(
                  <div key={i} className="flex gap-2 mb-2">
                    <select value={o.user_id} onChange={e=>{const a=[...matterForm.originators];a[i].user_id=e.target.value;setMatterForm({...matterForm,originators:a})}} className="flex-1 px-3 py-2 border border-gray-300 rounded-lg text-sm bg-white"><option value="">{L.select}</option>{partners.map(u=><option key={u.id} value={u.id}>{u.full_name}</option>)}</select>
                    <input type="number" value={o.percentage} onChange={e=>{const a=[...matterForm.originators];a[i].percentage=parseFloat(e.target.value)||0;setMatterForm({...matterForm,originators:a})}} className="w-20 px-3 py-2 border border-gray-300 rounded-lg text-sm text-center"/>
                ...
                <button onClick={()=>setMatterForm({...matterForm,originators:[...matterForm.originators,{user_id:'',percentage:100}]})} className="text-sm text-vexa-500 hover:text-vexa-600 font-medium">{L.addOrig}</button>
              </div>
              ...
              <div className="border-t pt-4 border-gray-100">
                <label className="flex items-center gap-3 cursor-pointer"><input type="checkbox" checked={matterForm.is_restricted} onChange={e=>setMatterForm({...matterForm,is_restricted:e.target.checked,restricted_associates:e.target.checked?matterForm.restricted_associates:[]})} className="rounded border-gray-300"/>
                  <div><span className="text-sm font-medium text-gray-900 flex items-center gap-1.5"><ShieldAlert size={14} className="text-amber-500"/>{L.restricted}</span><p className="text-xs text-gray-400 mt-0.5">{L.restrictedDesc}</p></div>
                </label>
              </div>
